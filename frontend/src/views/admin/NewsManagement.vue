        ...
        >
          <template #default>
            <div style="display: flex; align-items: center; gap: 10px;">
              ...
              <el-button size="small" type="danger" @click="batchDelete" :loading="batchDeleting">
...
import {
  ...
  batchDeleteNews as batchDeleteNewsApi,
...
async function batchDelete() {
  ...
  try {
    await ElMessageBox.confirm(
      `确定要删除选中的 ${selectedNews.value.length} 条新闻吗？删除后无法恢复！`,
      '批量删除确认',
      {
    ...
    const result = await batchDeleteNewsApi(ids)
    
    ElMessage.success(`批量删除成功！删除了 ${result.deletedCount} 条新闻`)
    clearSelection()
    await loadNews()
